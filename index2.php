...
<head>
    ...
    <title>MyTicket | Pemesanan Tiket Spesial PKM</title><!--begin::Primary Meta Tags-->
                            ...
                            <div class="card mb-6">
                                <div class="card-header">
                                    <div class="d-flex align-items-center justify-content-between">
                                        <h3 class="mb-0">Pemesanan Tiket Spesial PKM</h3>
                                        <style>
                                            .custom-orange {
...
$(document).ready(function() {
    ...
    $('#orderForm').on('submit', function(e) {
        ...
        // Validasi apakah data sudah diisi
        if (username && ticketId) {
            $.ajax({
                type: 'POST',
                url: 'check_voucher.php',  // Script untuk memeriksa voucher dan tiket
                data: { username: username, ticketId: ticketId },
                success: function(response) {
                    // Reset class dari hasil pesan
                    $('#orderResult').removeClass('d-none alert-danger alert-success alert-info');

                    // Cek isi response dan tampilkan hasilnya
                    if (response.toLowerCase().includes("diskon")) {
                        // Jika ada diskon
                        $('#orderResult').addClass('alert-success').html(response);
                    } else if (response.toLowerCase().includes("tiket tidak ditemukan")) {
                        // Jika tiket tidak ditemukan
                        $('#orderResult').addClass('alert-danger').html(response);
                    } else {
                        // Default response
                        $('#orderResult').addClass('alert-info').html(response);
                    }
                },
                error: function() {
